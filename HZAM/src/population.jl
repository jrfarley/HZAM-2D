...
function new_location(x::Float32, y::Float32, sigma_disp::Real)::Tuple{Float32, Float32}
	...
	dist = min(sigma_disp * randn(), 0.99)
	dir = rand() * 2 * pi
...
- `ideal_densities::Dict{Tuple{Float32, Float32}, Real}`: the expected local densities, based on geographically even distribution of individuals at carrying capacity.
...
- PopulationData(
	...
	sigma_comp::Real,
	intrinsic_R::Real,
	ideal_densities::Dict{Tuple{Float32, Float32}, Real}
)
```
# Details on behaviour of different constructors
...
struct PopulationData
	...
	function PopulationData(
		...
		ideal_densities = calc_ideal_densities(K_total, sigma_comp, 3*sigma_comp)
		...
		new(zones, growth_rates_F, ideal_densities)
	end
...
function calc_ideal_densities(
	...
	sigma_comp::Real,
	max_dist::Real,
)::Dict{Tuple{Float32, Float32}, <:Real}
	# the locations at which the ideal densities are calculated
	location_indices = [(x, y) for x in 0.0f0:0.01f0:1.0f0 for y in 0.0f0:0.01f0:1.0f0]

	function calc_ideal_density(location::Tuple{Float32, Float32})
		x=location[1]
		y=location[2]
	...
	return Dict(location_indices .=> calc_ideal_density.(location_indices))
end
...
function calc_growth_rates(
	...
	function get_ideal_density(x, y)
		return ideal_densities[(round(x, digits = 2), round(y, digits = 2))]
	end

	ideal_densities = get_ideal_density.(
		population[zone_index].x_locations_F,
		population[zone_index].y_locations_F,
	)
